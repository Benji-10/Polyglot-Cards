...
export default function SettingsPage() {
  ...
  const testDb = async () => {
    ...
    try {
      ...
      if (res.ok) { setDbStatus('ok'); toast.success('Database connection OK') }
      else         { setDbStatus('error'); toast.error(`DB error: ${res.status}`) }
  ...
  const testGemini = async () => {
    ...
    try {
      ...
      const res = await fetch('/.netlify/functions/generate', {
        ...
        body: JSON.stringify({ vocab: { vocab: ['test'], targetLanguage: 'Korean' }, blueprint: [] }),
      })
      if (res.ok) { setGeminiStatus('ok'); toast.success('Gemini API key is working') }
      else {
        const b = await res.json().catch(() => ({}))
        setGeminiStatus('error'); toast.error(b.error || `Gemini error: ${res.status}`)
      }
    ...
    <div className="max-w-2xl mx-auto px-6 py-10">
      ...
      <Section title="Study Defaults">
        <SettingRow label="Default source language" desc="The language you already know">
          <select className="input w-36 text-sm py-1.5"
            value={settings.defaultSourceLanguage || 'English'}
      ...
      <Section title="Connections">
        <div className="text-xs mb-4" style={{ color: 'var(--text-muted)' }}>
          API keys live in Netlify environment variables — never exposed to the browser.
        </div>
...
function ThemeSection({ settings, updateSettings }) {
  ...
  const updateCustomVar = (key, val) => {
    const next = { ...customTheme, [key]: val }
    updateSettings({ theme: 'custom', customTheme: next })
    setShowCustom(true)
  }
      ...
      <div>
        ...
        <div className="flex flex-wrap gap-2">
          ...
          <button onClick={selectCustom}
            ...
            <div className="flex gap-0.5 rounded overflow-hidden items-center justify-center" style={{ height: '20px', width: '48px', background: 'var(--bg-elevated)', fontSize: '12px' }}>
              🎨
            </div>
            ...
            {CUSTOM_THEME_KEYS.map(({ key, label }) => (
              <div key={key} className="flex items-center justify-between gap-3">
                ...
                <div className="flex items-center gap-2">
                  <input type="color" value={customTheme[key] || '#000000'}
                    onChange={e => updateCustomVar(key, e.target.value)}
                    className="w-8 h-8 rounded cursor-pointer border-0 p-0"
                    style={{ background: 'transparent' }} />
                  <input type="text" value={customTheme[key] || ''}
                    onChange={e => /^#[0-9a-fA-F]{0,6}$/.test(e.target.value) && updateCustomVar(key, e.target.value)}
                    className="input font-mono text-xs py-1 w-24 text-center" />
                </div>
              </div>
            ))}
          </div>
          <button className="btn-secondary text-xs mt-3" onClick={() => updateSettings({ customTheme: DEFAULT_CUSTOM_THEME })}>
  ...
  return (
    <div className="space-y-3">
      <p className="text-xs" style={{ color: 'var(--text-muted)' }}>
        These fields appear as quick-add suggestions in the Blueprint editor. Edit them here to match your learning style.
      </p>

      <div className="space-y-2">
        {fields.map((field, idx) => (
          <QuickAddRow key={idx} field={field}
      ...
      <div className="flex gap-2">
        ...
        <button className="btn-ghost text-xs py-1.5 px-3"
          ...
          onClick={() => { if (window.confirm('Reset quick-add fields to defaults?')) setFields(DEFAULT_QUICK_ADD) }}>
          Reset to defaults
        </button>
      </div>
    </div>
  ...
  return (
    <div className="flex items-center justify-between gap-4 p-3 rounded-xl"
      ...
      <div className="flex items-center gap-3 flex-shrink-0">
        ...
        <button className="btn-secondary text-xs py-1.5 px-3" disabled={status === 'checking'} onClick={onTest}>
